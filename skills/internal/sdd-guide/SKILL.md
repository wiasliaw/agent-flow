...
This skill defines the spec-driven-development (SDD) format contract used
across the Spec, Ticket, and Wrap phases: the EARS sentence pattern for
requirement text, the delta-spec ADDED/MODIFIED/REMOVED contract, the
...
(see the `state-management` skill); the orchestrator mirrors this field
into `state.json` whenever it reads a change to the ticket's frontmatter.

- Every ticket must reference at least one requirement number.
- Test cases produced in the Ticket phase should also note the
  corresponding requirement number in the test file's comments or
  description, so a human can trace the test back to its requirement.

## 4. Merging a delta into the main spec (Wrap phase)

This is performed by `wrap-executor`. The main spec is stored per domain at
`.agent-flow/specs/<domain>/spec.md`. Domain-splitting granularity is
project-defined; this skill does not prescribe how domains are split.
...
`spec-reviewer` and `review-spec-compliance-auditor` must additionally
verify:
